...
matriz <-matrix(c(1,2,3,4),nrow = 2)  # matrices
error <- rnorm(1000, sd=1 ,mean = 0)             # un vector con la distribucion normal
y_vec <- 2 * x_vec + error[1:11]      # y es igual a 2x + un error normal
...
c_impares <- oddcount(x_vec)      # contar impares en X_vec con la funcion que defini
